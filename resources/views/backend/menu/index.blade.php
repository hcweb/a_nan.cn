            ...
            <div class="pad-btm form-inline">
                <div class="row">
                    <div class="col-sm-6 table-toolbar-left">
                        <a href="{{route('menu.create')}}" class="btn btn-primary mar-rgt"><i
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" class="form-control text-center" value="{{$v->order}}"
                                           style="width: 50px;"
                                           onblur='updateOrderById("{{route('menu.update.order')}}","{{$v->id}}",this.value)'>
                                </td>
                                <td><i class="{{$v->icon_class}} fa-1x"></i></td>
                                <td>
                                    <i style="margin-right: {{$v->level*10}}px"></i>
                                    <i class="fa fa-file text-muted"></i>
                                    <span style="margin-right: 5px;">{{str_repeat('-',$v->level*5)}}</span>
                                    {{$v->title}}
                                </td>
                                <td>
                                    @switch($v->target)
                                ...
                                <td>{{$v->route}}</td>
                                <td>
                                    <a href="{{route('menu.edit',['id'=>$v->id])}}" class="btn btn-mint mar-rgt"><i
